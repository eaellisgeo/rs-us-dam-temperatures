First File: Export_Combined_RWC_Data_by_Dam.ipynb

To begin, RWC raw outputs are combined by dam (not snapped to any nodes) 
-- Instead, points are snapped based on the USGS gage location for the accuracy assessment. Output is a CSV file for each dam with all the 
...
Second File: Accuracy_Assessment.ipynb
...
Filtering is completed to remove empty gages and errors. Associated dams are selected with a spatial join in ArcGIS.
...
To get absolute accuracy, the gages used in the relative accuracy assessment are used. 
The temperature at the gage and the Landsat observation are directly compared to calculate absolute differences. 

This is used to get the RMSE, MAE, and Bias, as well as to create the graphics for both relative and absolute accuracies. 